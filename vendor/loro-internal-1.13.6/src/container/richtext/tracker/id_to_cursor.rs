...
impl IdToCursor {
    ...
    pub(super) fn external_delete_cursors(&self) -> Result<Vec<(ID, IdSpan)>, String> {
        let mut deletes = Vec::new();
        for (peer, fragments) in &self.map {
            for fragment in fragments {
                match &fragment.cursor {
                    Cursor::Insert(_) => {}
                    Cursor::Delete(target) => {
                        deletes.push((ID::new(*peer, fragment.counter), *target));
                    }
                    Cursor::Move { .. } => {
                        return Err(
                            "rich-text tracker contains unsupported move metadata".to_string()
                        );
                    }
                }
            }
        }
        deletes.sort_unstable_by_key(|(id, _)| *id);
        Ok(deletes)
    }
