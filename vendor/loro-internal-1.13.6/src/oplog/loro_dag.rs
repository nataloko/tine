...
use crate::external_store::{encode_causal_snapshot, CausalDagNode, CausalDagSnapshot};
...
impl AppDag {
    ...
    pub(crate) fn set_version_by_fast_snapshot_import(&mut self, v: BatchDecodeInfo) {
        ...
        if let Some(causal_dag) = v.causal_dag {
            assert_eq!(causal_dag.vv, v.vv);
            assert_eq!(causal_dag.frontiers, v.frontiers);
            let mut map = self.map.lock();
            for node in causal_dag.nodes {
                let node = AppDagNode::new(AppDagNodeInner {
                    ...
                    deps: node.deps,
                    vv: OnceCell::with_value(node.vv),
                    has_succ: node.has_succ,
                    len: node.len,
                });
                assert!(map.insert(node.id_start(), node).is_none());
            }
    ...
    pub(crate) fn evict_parsed_cache(&mut self) {
        assert!(
            self.pending_txn_node.is_none(),
            "cannot evict the DAG cache during a transaction"
        );
        // External documents retain compact causal summaries here. Parsed
        // change blocks are evicted separately; dropping this index would make
        // old-base joins reconstruct a linear branch block by block.
        debug_assert!(self.unparsed_vv.lock().is_empty());
        self.unhandled_dep_points.lock().clear();
    }

    pub(crate) fn encode_external_causal_snapshot(&self) -> LoroResult<bytes::Bytes> {
        if self.pending_txn_node.is_some() || !self.unparsed_vv.lock().is_empty() {
            return Err(LoroError::Unknown(
                "external causal metadata requires a fully materialized DAG".into(),
            ));
        }

        let nodes = self.map.lock().values().cloned().collect::<Vec<_>>();
        let nodes = nodes
            .into_iter()
            .map(|node| CausalDagNode {
                peer: node.peer,
                cnt: node.cnt,
                lamport: node.lamport,
                deps: node.deps.clone(),
                vv: self.ensure_vv_for(&node),
                has_succ: node.has_succ,
                len: node.len,
                boundary_proof: None,
            })
            .collect();
        encode_causal_snapshot(CausalDagSnapshot {
            vv: self.vv.clone(),
            frontiers: self.frontiers.clone(),
            nodes,
        })
