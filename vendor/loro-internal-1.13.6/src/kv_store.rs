...
pub type KvStoreHandle = Arc<Mutex<dyn KvStore>>;

pub fn kv_store_handle(store: impl KvStore + 'static) -> KvStoreHandle {
...
pub trait KvStore: std::fmt::Debug + Send + Sync {
    ...
    /// Make prior writes durable when the implementation has a durability
    /// boundary. In-memory stores keep the no-op default.
    fn flush(&mut self) {}

    /// Return and clear a sticky backend error recorded by an otherwise
    /// infallible trait method.
    ///
    /// Existing implementations remain source-compatible through this
    /// default. External stores should record authentication, I/O, and data
    /// integrity failures and return them here.
