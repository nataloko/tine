...
pub use loro_internal::kv_store::{kv_store_handle, KvStore, KvStoreHandle, MemKvStore};
...
impl LoroDoc {
    ...
    /// Open a document backed by an injected external change store.
    ///
    /// Pass `None` only for an empty store. Reopening a populated store
    /// requires the exact checkpoint returned with the current store metadata
    /// by [`LoroDoc::flush_external_store`]. A stale, newer, or mixed
    /// checkpoint/store pair is rejected; lagging checkpoint replay is not
    /// supported. The caller must authenticate and atomically publish the
    /// checkpoint with the external store root. Neither artifact is trusted
    /// without the other. Reopen verifies compact sliceable causal proofs and
    /// owner-bound complete text-tracker commitments without reading or
    /// decoding historical change blocks. The authenticated caller root remains
    /// responsible for binding historical block contents and the checkpoint
    /// pair; replacing that root is outside this API's threat contract.
    pub fn from_external_store(
        checkpoint: Option<&[u8]>,
        external_kv: KvStoreHandle,
    ) -> LoroResult<Self> {
        let inner = InnerLoroDoc::from_external_store(checkpoint, external_kv)?;
        inner.start_auto_commit();
        Ok(Self::_new(inner))
    }
    ...
    /// Durably flush the injected external change store and return a compact
    /// state checkpoint paired to the exact resulting store metadata.
    ///
    /// The caller should authenticate and atomically publish the returned
    /// checkpoint with the resulting external store root only after this
    /// method succeeds. Flush first verifies that `xm` still matches the exact
    /// predecessor digest loaded with the authenticated checkpoint. It slices
    /// and recomposes prior causal source proofs, validates every newly
    /// uncovered change boundary, and advances complete owner-bound text
    /// trackers only through newly collected operations before publishing
    /// canonical metadata.
    pub fn flush_external_store(&self) -> LoroResult<Vec<u8>> {
        self.doc.flush_external_store()
    }

    /// Flush and evict parsed change blocks, DAG nodes, and history caches.
    pub fn evict_external_store_cache(&self) -> LoroResult<()> {
        self.doc.evict_external_store_cache()
    }

    /// Return and clear a sticky error reported by the injected store.
    pub fn take_external_store_error(&self) -> Option<LoroError> {
        self.doc.take_external_store_error()
    }
